...
function sortByRank<T extends { id: number; rank: number }>(items: readonly T[]): T[] {
...
function getPreferredTranslation<T extends { langId: number }>(
  ...
  return (
    ...
    translations.find(translation => translation.langId === localeToLangId.en) ??
    translations[0] ??
    null
  )
}
...
function getVisibleArticles(category: RecursiveArticleCategoryDto | null): RecursiveArticleDto[] {
  if (!category) {
    return []
  }
...
function getChildCategoryByRank(
  ...
  return getVisibleChildren(category).find(child => child.rank === rank) ?? null
}
...
function getArticleByRank(
  ...
  return getVisibleArticles(category).find(article => article.rank === rank) ?? null
}
...
function parseRatingMeta(extraInfo: string | null): { ratingValue: string; ratingCount: string } {
...
function toActionContent(article: RecursiveArticleDto | null, langId: number, fallback: HomeActionContent): HomeActionContent {
  if (!article) {
    return fallback
  }
...
function toLinkItemContent(article: RecursiveArticleDto, langId: number): HomeLinkItemContent {
  const translation = getArticleTranslation(article, langId)
...
function mapHeroStats(
  ...
  return articles.map((article, index) => {
    ...
    const parsed = parseStatValue(localized.name) ?? parseStatValue(english.name)
  ...
  return {
    ...
    app: {
      ...
      downloadLinks: [
        ...
        { label: 'Google Play', path: '/download/android', iconKey: 'icon-google-play' },
      ],
    },
    academy: {
    ...
    business: {
      ...
      primaryAction: { label: home.business.primaryCta, path: '/b2b/open-corporate-account' },
      secondaryAction: { label: home.business.secondaryCta, path: '/b2b/book-demo' },
    },
    testimonial: {
    ...
    footer: {
      ...
      pages: {
        ...
        links: [
          ...
          { label: home.footer.links[2] ?? '', path: '#academy' },
      ...
      contact: {
        ...
        links: [
          { label: 'info@mwafq.com', path: 'mailto:info@mwafq.com', iconKey: 'icon-email' },
          { label: '+966 5 400000', path: 'tel:+9665400000', iconKey: 'icon-phone' },
          { label: 'Riyadh - KSA', path: '/contact#riyadh', iconKey: 'icon-location' },
        ],
      },
    },
  }
...
function mapHeaderContent(
  ...
  const headerCategory = getChildCategoryByRank(rootCategory, HOME_SECTION_RANKS.header)

  if (!headerCategory) {
    return fallback
  }

  const brandArticle = getArticleByRank(headerCategory, HEADER_ARTICLE_RANKS.brand)
  const primaryArticle = getArticleByRank(headerCategory, HEADER_ARTICLE_RANKS.primaryAction)
  const signInArticle = getArticleByRank(headerCategory, HEADER_ARTICLE_RANKS.signIn)
  const localeSwitchArticle = getArticleByRank(headerCategory, HEADER_ARTICLE_RANKS.localeSwitch)
  const navLinks = getVisibleArticles(headerCategory)
    .filter(article => article.rank >= 10 && article.rank < HEADER_ARTICLE_RANKS.primaryAction)
    .map(article => toLinkItemContent(article, langId))

  const brandTranslation = brandArticle ? getArticleTranslation(brandArticle, langId) : null

  return {
    brandLabel: trimToNull(brandTranslation?.name) ?? fallback.brandLabel,
    brandDescription: trimToNull(brandTranslation?.description) ?? fallback.brandDescription,
    brandPath: trimToNull(brandArticle?.path) ?? fallback.brandPath,
    brandImageSrc: resolveCmsAssetUrl(brandArticle?.image) ?? fallback.brandImageSrc,
    navLinks: navLinks.length > 0 ? navLinks : fallback.navLinks,
    primaryAction: primaryArticle
      ? toActionContent(primaryArticle, langId, fallback.primaryAction ?? { label: '', path: null })
...
function mapHeroContent(
  ...
  const heroCategory = getChildCategoryByRank(rootCategory, HOME_SECTION_RANKS.hero)

  if (!heroCategory) {
    return fallback
  }

  const contentArticle = getArticleByRank(heroCategory, HERO_ARTICLE_RANKS.content)
  const contentTranslation = contentArticle
    ? getArticleTranslation(contentArticle, langId)
    : null
  const wordsCategory = getChildCategoryByRank(heroCategory, HERO_CHILD_CATEGORY_RANKS.words)
  const wordsArticle = getArticleByRank(wordsCategory, 1)
  const wordsTranslation = wordsArticle ? getArticleTranslation(wordsArticle, langId) : null
  const actionsCategory = getChildCategoryByRank(heroCategory, HERO_CHILD_CATEGORY_RANKS.actions)
  const primaryAction = getArticleByRank(actionsCategory, 10)
  const secondaryAction = getArticleByRank(actionsCategory, 20)
  const metricsCategory = getChildCategoryByRank(heroCategory, HERO_CHILD_CATEGORY_RANKS.metrics)
  const phoneUiCategory = getChildCategoryByRank(heroCategory, HERO_CHILD_CATEGORY_RANKS.phoneUi)
  ...
  const phoneTilesCategory = getChildCategoryByRank(heroCategory, HERO_CHILD_CATEGORY_RANKS.phoneTiles)
  const floatingCardsCategory = getChildCategoryByRank(heroCategory, HERO_CHILD_CATEGORY_RANKS.floatingCards)
  ...
  return {
    ...
    badgeImages:
      splitImageList(contentArticle?.images, fallback.badge || fallback.titleLead).length > 0
        ? splitImageList(contentArticle?.images, fallback.badge || fallback.titleLead)
        : fallback.badgeImages,
    titleLead: trimToNull(contentTranslation?.name) ?? fallback.titleLead,
    titleMiddle: trimToNull(contentTranslation?.extraInfo) ?? fallback.titleMiddle,
    rotatingWords:
      wordsTranslation
        ? [
    ...
    primaryAction: toActionContent(primaryAction, langId, fallback.primaryAction),
    secondaryAction: toActionContent(secondaryAction, langId, fallback.secondaryAction),
    stats: mapHeroStats(metricsCategory, langId, fallback.stats),
    phoneGreeting: trimToNull(greetingTranslation?.name) ?? fallback.phoneGreeting,
    phoneName: trimToNull(greetingTranslation?.extraInfo) ?? fallback.phoneName,
    phoneSearchPlaceholder:
      trimToNull(greetingTranslation?.shortDescription) ?? fallback.phoneSearchPlaceholder,
    servicesTitle: trimToNull(servicesTranslation?.name) ?? fallback.servicesTitle,
    servicesLink: trimToNull(servicesTranslation?.extraInfo) ?? fallback.servicesLink,
    phoneTiles:
    ...
    liveBookingsLabel: trimToNull(liveTranslation?.description) ?? fallback.liveBookingsLabel,
    floatingCards:
...
function mapServicesContent(
  ...
  const servicesCategory = getChildCategoryByRank(rootCategory, HOME_SECTION_RANKS.services)

  if (!servicesCategory) {
    return fallback
  }

  const headerArticle = getArticleByRank(servicesCategory, SERVICES_ARTICLE_RANKS.header)
  ...
  return {
    eyebrow: trimToNull(headerTranslation?.shortDescription) ?? fallback.eyebrow,
...
function mapWhyContent(
  ...
  const whyCategory = getChildCategoryByRank(rootCategory, HOME_SECTION_RANKS.why)
  ...
  const headerTranslation = headerArticle ? getArticleTranslation(headerArticle, langId) : null
...
function mapBookingContent(
  ...
  const bookingCategory = getChildCategoryByRank(rootCategory, HOME_SECTION_RANKS.booking)

  if (!bookingCategory) {
    return fallback
  }

  const headerArticle = getArticleByRank(bookingCategory, BOOKING_ARTICLE_RANKS.header)
  const headerTranslation = headerArticle ? getArticleTranslation(headerArticle, langId) : null
  const fieldsCategory = getChildCategoryByRank(bookingCategory, BOOKING_CHILD_CATEGORY_RANKS.fields)
  const optionsCategory = getChildCategoryByRank(bookingCategory, BOOKING_CHILD_CATEGORY_RANKS.options)
  ...
  return {
    ...
    fields: {
      exam: {
        label:
          trimToNull(examField ? getArticleTranslation(examField, langId).name : null) ??
          fallback.fields.exam.label,
        placeholder:
          trimToNull(examField ? getArticleTranslation(examField, langId).shortDescription : null) ??
          fallback.fields.exam.placeholder,
      },
      city: {
        label:
          trimToNull(cityField ? getArticleTranslation(cityField, langId).name : null) ??
          fallback.fields.city.label,
        placeholder:
          trimToNull(cityField ? getArticleTranslation(cityField, langId).shortDescription : null) ??
          fallback.fields.city.placeholder,
      },
      date: {
        label:
          trimToNull(dateField ? getArticleTranslation(dateField, langId).name : null) ??
          fallback.fields.date.label,
        placeholder:
          trimToNull(dateField ? getArticleTranslation(dateField, langId).shortDescription : null) ??
    ...
    examOptions:
      getVisibleArticles(optionsCategory).length > 0
        ? getVisibleArticles(optionsCategory).map(article => getArticleTranslation(article, langId).name)
...
function mapStepsContent(
  ...
  const stepsCategory = getChildCategoryByRank(rootCategory, HOME_SECTION_RANKS.steps)

  if (!stepsCategory) {
    return fallback
  }

  const headerArticle = getArticleByRank(stepsCategory, STEPS_ARTICLE_RANKS.header)
    ...
    items:
      stepCategories.every(Boolean)
        ? stepCategories.map((category, index) => {
            ...
            return {
              title:
                trimToNull(contentArticle ? getArticleTranslation(contentArticle, langId).name : null) ??
                fallbackItem?.title ??
                '',
              body:
                trimToNull(contentArticle ? getArticleTranslation(contentArticle, langId).description : null) ??
                fallbackItem?.body ??
                '',
              meta1: {
                value:
                  trimToNull(meta1Article ? getArticleTranslation(meta1Article, langId).name : null) ??
                  fallbackItem?.meta1.value ??
                  '',
                label:
                  trimToNull(meta1Article ? getArticleTranslation(meta1Article, langId).description : null) ??
                  fallbackItem?.meta1.label ??
                  '',
              },
              meta2: {
                value:
                  trimToNull(meta2Article ? getArticleTranslation(meta2Article, langId).name : null) ??
                  fallbackItem?.meta2.value ??
                  '',
                label:
                  trimToNull(meta2Article ? getArticleTranslation(meta2Article, langId).description : null) ??
...
function mapAppContent(
  ...
  const appCategory = getChildCategoryByRank(rootCategory, HOME_SECTION_RANKS.app)
  ...
  const headerTranslation = headerArticle ? getArticleTranslation(headerArticle, langId) : null
  const scheduleCategory = getChildCategoryByRank(appCategory, APP_CHILD_CATEGORY_RANKS.schedule)
  const statusCategory = getChildCategoryByRank(appCategory, APP_CHILD_CATEGORY_RANKS.status)
  const reportsCategory = getChildCategoryByRank(appCategory, APP_CHILD_CATEGORY_RANKS.reports)
  const pointsCategory = getChildCategoryByRank(appCategory, APP_CHILD_CATEGORY_RANKS.points)
  const linksCategory = getChildCategoryByRank(appCategory, APP_CHILD_CATEGORY_RANKS.links)
  ...
  const reportItems = getVisibleArticles(reportsCategory).filter(article => article.rank > 1)
  ...
  return {
    ...
    scheduleCard: {
      label:
        trimToNull(scheduleHeader ? getArticleTranslation(scheduleHeader, langId).name : null) ??
        fallback.scheduleCard.label,
      detail:
        trimToNull(scheduleHeader ? getArticleTranslation(scheduleHeader, langId).description : null) ??
        fallback.scheduleCard.detail,
      appointment: {
        value:
          trimToNull(scheduleAppointment ? getArticleTranslation(scheduleAppointment, langId).name : null) ??
          fallback.scheduleCard.appointment.value,
        detail:
          trimToNull(scheduleAppointment ? getArticleTranslation(scheduleAppointment, langId).description : null) ??
          fallback.scheduleCard.appointment.detail,
        location:
          trimToNull(scheduleAppointment ? getArticleTranslation(scheduleAppointment, langId).shortDescription : null) ??
          fallback.scheduleCard.appointment.location,
        iconKey: extractIconKey(scheduleAppointment?.image) ?? fallback.scheduleCard.appointment.iconKey,
      },
    },
    statusCard: {
      label:
        trimToNull(statusHeader ? getArticleTranslation(statusHeader, langId).name : null) ??
        fallback.statusCard.label,
      detail:
        trimToNull(statusHeader ? getArticleTranslation(statusHeader, langId).description : null) ??
        fallback.statusCard.detail,
      status:
        trimToNull(statusHeader ? getArticleTranslation(statusHeader, langId).shortDescription : null) ??
        fallback.statusCard.status,
    },
    reportsCard: {
      label:
        trimToNull(reportsHeader ? getArticleTranslation(reportsHeader, langId).name : null) ??
        fallback.reportsCard.label,
      detail:
        trimToNull(reportsHeader ? getArticleTranslation(reportsHeader, langId).description : null) ??
        fallback.reportsCard.detail,
      status:
        trimToNull(reportsHeader ? getArticleTranslation(reportsHeader, langId).shortDescription : null) ??
...
function mapAcademyContent(
  ...
  const academyCategory = getChildCategoryByRank(rootCategory, HOME_SECTION_RANKS.academy)

  if (!academyCategory) {
    return fallback
  }

  const headerArticle = getArticleByRank(academyCategory, ACADEMY_ARTICLE_RANKS.header)
  const ctaArticle = getArticleByRank(academyCategory, ACADEMY_ARTICLE_RANKS.cta)
  const headerTranslation = headerArticle ? getArticleTranslation(headerArticle, langId) : null
  ...
  return {
    ...
    title: trimToNull(headerTranslation?.name) ?? fallback.title,
    accent: trimToNull(headerTranslation?.extraInfo) ?? fallback.accent,
    ctaLabel: trimToNull(ctaArticle ? getArticleTranslation(ctaArticle, langId).name : null) ?? fallback.ctaLabel,
    items: (() => {
...
function mapStatsContent(
  ...
  const statsCategory = getChildCategoryByRank(rootCategory, HOME_SECTION_RANKS.stats)

  if (!statsCategory) {
    return fallback
  }

  const headerArticle = getArticleByRank(statsCategory, STATS_ARTICLE_RANKS.header)
  const headerTranslation = headerArticle ? getArticleTranslation(headerArticle, langId) : null
  const items = getVisibleArticles(statsCategory)
    ...
    .map((article, index) => {
      const translation = getArticleTranslation(article, langId)
      const english = getArticleTranslation(article, localeToLangId.en)
      const parsed = parseStatValue(translation.name) ?? parseStatValue(english.name)
      const fallbackItem = fallback.items[index]
...
function mapBusinessContent(
  ...
  const businessCategory = getChildCategoryByRank(rootCategory, HOME_SECTION_RANKS.business)

  if (!businessCategory) {
    return fallback
  }

  const headerArticle = getArticleByRank(businessCategory, BUSINESS_ARTICLE_RANKS.header)
  const primaryActionArticle = getArticleByRank(businessCategory, BUSINESS_ARTICLE_RANKS.primaryAction)
  const secondaryActionArticle = getArticleByRank(businessCategory, BUSINESS_ARTICLE_RANKS.secondaryAction)
  const headerTranslation = headerArticle ? getArticleTranslation(headerArticle, langId) : null
  ...
  const tabsCategory = getChildCategoryByRank(businessCategory, BUSINESS_CHILD_CATEGORY_RANKS.tabs)
  const tabsArticle = getArticleByRank(tabsCategory, 1)
  const tabsTranslation = tabsArticle ? getArticleTranslation(tabsArticle, langId) : null
  const metricsCategory = getChildCategoryByRank(businessCategory, BUSINESS_CHILD_CATEGORY_RANKS.metrics)
  const employeesCategory = getChildCategoryByRank(businessCategory, BUSINESS_CHILD_CATEGORY_RANKS.employees)
  ...
  return {
    ...
    points:
      pointArticles.length > 0
        ? pointArticles.map(article => trimToNull(getArticleTranslation(article, langId).name) ?? '')
        : fallback.points,
    tabs:
      tabsTranslation
        ? [
    ...
    employees:
      getVisibleArticles(employeesCategory).length > 0
        ...
        : fallback.employees,
    primaryAction: toActionContent(primaryActionArticle, langId, fallback.primaryAction),
    secondaryAction: toActionContent(secondaryActionArticle, langId, fallback.secondaryAction),
  }
}
...
function mapTestimonialContent(
  ...
  const testimonialCategory = getChildCategoryByRank(rootCategory, HOME_SECTION_RANKS.testimonial)
  const article = getArticleByRank(testimonialCategory, TESTIMONIAL_ARTICLE_RANKS.content)
...
function mapFinalCtaContent(
  ...
  const finalCtaCategory = getChildCategoryByRank(rootCategory, HOME_SECTION_RANKS.finalCta)

  if (!finalCtaCategory) {
    return fallback
  }

  const contentArticle = getArticleByRank(finalCtaCategory, FINAL_CTA_ARTICLE_RANKS.content)
  const primaryActionArticle = getArticleByRank(finalCtaCategory, FINAL_CTA_ARTICLE_RANKS.primaryAction)
  const secondaryActionArticle = getArticleByRank(finalCtaCategory, FINAL_CTA_ARTICLE_RANKS.secondaryAction)
  const translation = contentArticle ? getArticleTranslation(contentArticle, langId) : null
...
function mapFooterGroup(
  ...
  const headingTranslation = headingArticle ? getArticleTranslation(headingArticle, langId) : null
...
function mapFooterContent(
  ...
  const footerCategory = getChildCategoryByRank(rootCategory, HOME_SECTION_RANKS.footer)

  if (!footerCategory) {
    return fallback
  }

  const brandArticle = getArticleByRank(footerCategory, FOOTER_ARTICLE_RANKS.brand)
  const newsletterArticle = getArticleByRank(footerCategory, FOOTER_ARTICLE_RANKS.newsletter)
  const copyrightArticle = getArticleByRank(footerCategory, FOOTER_ARTICLE_RANKS.copyright)
  const brandTranslation = brandArticle ? getArticleTranslation(brandArticle, langId) : null
  const newsletterTranslation = newsletterArticle ? getArticleTranslation(newsletterArticle, langId) : null
  const copyrightTranslation = copyrightArticle ? getArticleTranslation(copyrightArticle, langId) : null
  ...
  return {
    ...
    brandPath: trimToNull(brandArticle?.path) ?? fallback.brandPath,
    brandImageSrc: resolveCmsAssetUrl(brandArticle?.image) ?? fallback.brandImageSrc,
    newsletterPlaceholder: trimToNull(newsletterTranslation?.name) ?? fallback.newsletterPlaceholder,
    newsletterEyebrow: trimToNull(newsletterTranslation?.shortDescription) ?? fallback.newsletterEyebrow,
    newsletterAction: trimToNull(newsletterTranslation?.extraInfo) ?? fallback.newsletterAction,
    copyrightLabel: trimToNull(copyrightTranslation?.name) ?? fallback.copyrightLabel,
    copyrightBody: trimToNull(copyrightTranslation?.description) ?? fallback.copyrightBody,
    ...
    contact: mapFooterGroup(
      getChildCategoryByRank(footerCategory, FOOTER_CHILD_CATEGORY_RANKS.contact),
      langId,
      fallback.contact,
    ),
  }
...
function buildHomePageContent(
  ...
  return {
    ...
    testimonial: mapTestimonialContent(fallback.testimonial, rootCategory, langId),
...
const fetchHomeContentTree = cache(async (): Promise<RecursiveArticleCategoryDto | null> => {
  ...
  try {
    ...
    console.error('[home-content] Failed to fetch recursive home content.', error)
    return null
  }
})
