...
export function HeroSection({ locale, content, isRtl }: Props) {
  ...
  const badgeImages = content.badgeImages.length > 0
    ? content.badgeImages
    : [
  ...
  return (
    <section
      id="home"
      className="relative overflow-hidden border-b-2 border-[#e5e7f0] bg-[#f4f4f6] px-4 pb-20 pt-70 md:px-7"
    >
        ...
        <div className="-translate-y-10">
          ...
          <h1 className="mb-7 text-[clamp(44px,6.5vw,92px)] font-extrabold leading-[1.05] tracking-[-2.6px] text-[#1e2364]">
            ...
            {hasSecondLine ? (
              <span className="block">
                ...
                {hasRotatingWords ? <RotatingWord words={content.rotatingWords} /> : null}
              </span>
            ) : null}
          </h1>
          ...
          <div className="mb-11 flex flex-wrap gap-3.5">
            <CmsLink
              ...
              className={buttonVariants({ variant: 'brand', size: 'hero', shape: 'pill' })}
            >
            ...
            <CmsLink
              ...
              className={buttonVariants({ variant: 'brandOutline', size: 'hero', shape: 'pill' })}
            >
          ...
          {content.floatingCards.slice(0, 3).map((card, index) => (
            ...
            >
              ...
              <div className={isRtl ? 'text-right' : ''}>
                ...
                <span className="block text-[11px] text-[#6b7196]">{card.detail}</span>
              </div>
            </div>
          ))}
          ...
          <div className="absolute left-1/2 top-1/2 h-[540px] w-[260px] animate-[phoneFloat_6s_ease-in-out_infinite] overflow-visible rounded-[42px] border-2 border-[#1e2364] bg-white shadow-[0_20px_60px_rgba(30,35,100,0.12)]">
            ...
            <div className="absolute inset-[14px] flex flex-col gap-[10px] overflow-hidden rounded-[36px] border-2 border-[#e5e7f0] bg-white px-[13px] pb-[14px] pt-6">
              <div className="mt-3.5 flex items-center justify-between">
                <div>
                  <p className="text-[11.5px] font-medium leading-none text-[#6b7196]">{content.phoneGreeting}</p>
                  <p className="mt-1.5 text-[17px] font-extrabold leading-none tracking-[-0.3px] text-[#1e2364]">{content.phoneName}</p>
                </div>
              ...
              <div className="flex items-center justify-between px-0.5">
                <span className="text-[11.5px] font-extrabold tracking-[-0.1px] text-[#1e2364]">{content.servicesTitle}</span>
                <span className="text-[10px] font-semibold text-[#00a8f1]">{content.servicesLink}</span>
              </div>
              ...
              <div className="grid grid-cols-2 gap-2.5">
                {content.phoneTiles.map((tile, index) => (
                  ...
                  >
                    <div className={`mb-2 flex h-10 w-10 items-center justify-center overflow-hidden rounded-[14px] ${phoneTileStyles[index]?.icon ?? 'bg-white text-[#1e2364]'}`}>
              ...
              <div className="mt-auto flex items-center gap-2.5 rounded-[14px] bg-[#1e2364] px-3.5 py-[11px] text-white">
                <span className="h-2 w-2 flex-shrink-0 rounded-full bg-[#00a8f1] shadow-[0_0_0_3px_rgba(0,168,241,0.25)] animate-[phLivePulse_1.6s_ease-in-out_infinite]" aria-hidden="true" />
                <span className="flex flex-1 items-baseline gap-1.5">
                  <strong className="text-[14px] font-extrabold tracking-[-0.3px] text-[#00a8f1]">{content.liveBookings}</strong>
                  <span className="text-[10.5px] opacity-85">{content.liveBookingsLabel}</span>
                </span>
