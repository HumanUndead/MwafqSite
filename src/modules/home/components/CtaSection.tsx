    ...
    <section className="relative flex min-h-screen flex-col items-center justify-center bg-[#1e2364] px-[60px] py-[100px] text-center">
      ...
      <h2 className="relative z-10 mb-[22px] text-[clamp(34px,5vw,64px)] font-extrabold leading-[1.04] tracking-[-1.8px] text-white">
        ...
        {content.highlight ? (
          <>
            <br />
            <em className="font-normal italic text-[#00a8f1]/95">{content.highlight}</em>
          </>
        ) : null}
      </h2>
      ...
      <div className="relative z-10 flex flex-wrap justify-center gap-3.5">
        <CmsLink locale={locale} href={content.primaryAction.path} className={buttonVariants({ variant: 'brandInverse', size: 'hero', shape: 'pill' })}>
          {content.primaryAction.label}
        </CmsLink>
        <CmsLink locale={locale} href={content.secondaryAction.path} className={buttonVariants({ variant: 'brandGhost', size: 'hero', shape: 'pill' })}>
