...
function DatePicker({
  ...
  const cells: Array<{ d: number; out?: boolean; isToday?: boolean; isSelected?: boolean }> = []
  ...
  for (let day = 1; day <= daysInMonth; day += 1) {
    const isToday = day === today.getDate() && viewM === today.getMonth() && viewY === today.getFullYear()
    const isSelected = value ? value.d === day && value.m === viewM && value.y === viewY : false
  ...
  const displayValue = value ? `${pad(value.d)} / ${pad(value.m + 1)} / ${value.y}` : null
  ...
  return (
    <div ref={wrapRef} className="relative w-full">
      ...
      >
        <span className={displayValue ? 'text-[#1e2364]' : 'text-[rgba(30,35,100,0.45)]'}>
      ...
      {open ? (
        <div role="dialog" className="absolute left-0 top-[calc(100%+4px)] z-[100] w-[232px] rounded-lg border border-[#e5e7f0] bg-white p-2.5 shadow-[0_10px_15px_-3px_rgba(0,0,0,0.05),0_4px_6px_-4px_rgba(0,0,0,0.05)]">
          <div className="mb-1.5 flex items-center justify-between px-0.5">
            <button type="button" onClick={() => { if (viewM === 0) { setViewM(11); setViewY(year => year - 1) } else setViewM(month => month - 1) }} className="flex h-6 w-6 items-center justify-center rounded border border-[#e5e7f0] bg-white text-[#1e2364] hover:border-[#00a8f1] hover:bg-[rgba(0,168,241,0.08)]">
            ...
            <button type="button" onClick={() => { if (viewM === 11) { setViewM(0); setViewY(year => year + 1) } else setViewM(month => month + 1) }} className="flex h-6 w-6 items-center justify-center rounded border border-[#e5e7f0] bg-white text-[#1e2364] hover:border-[#00a8f1] hover:bg-[rgba(0,168,241,0.08)]">
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><polyline points="9 18 15 12 9 6" /></svg>
            </button>
          </div>
          <div className="mb-0.5 grid grid-cols-7 gap-0.5">
            {WEEKDAYS.map(day => (
              <span key={day} className="py-1 text-center text-[10px] font-semibold uppercase tracking-[0.04em] text-[rgba(30,35,100,0.45)]">{day}</span>
            ))}
          </div>
            ...
            {cells.map((cell, index) => (
              <button
                ...
                className={[
                  ...
                  cell.isSelected ? '!bg-transparent !font-bold !text-[#00a8f1]' : '',
                ].join(' ')}
              >
                {cell.d}
              </button>
...
function ExamSelect({
  ...
  return (
    <div ref={wrapRef} className="relative w-full">
      ...
      {open ? (
        <ul role="listbox" className="absolute left-0 top-[calc(100%+4px)] z-[100] max-h-96 w-full list-none overflow-y-auto rounded-lg border border-[#e5e7f0] bg-white p-1 shadow-[0_10px_15px_-3px_rgba(0,0,0,0.05)]">
          {options.map(option => (
            <li
              key={option}
              role="option"
    ...
    <section id="booking" className="relative z-50 px-4 pb-[60px] pt-[100px] md:px-7">
      ...
      <div className="relative mx-auto max-w-[1100px]">
        <div className="relative overflow-visible rounded-[32px] border-2 border-[#1e2364] bg-white px-14 pb-[90px] pt-[46px] text-center">
          ...
          <form onSubmit={event => event.preventDefault()} className="relative z-[5] grid items-end gap-[18px] text-left md:grid-cols-[1.2fr_1.2fr_1fr_auto]">
            <div>
              <label className="mb-2 ml-1.5 block text-[13px] font-bold text-[#1e2364]">{content.fields.exam.label}</label>
              <ExamSelect
                value={exam}
                onChange={setExam}
            ...
            <div>
              <label className="mb-2 ml-1.5 block text-[13px] font-bold text-[#1e2364]">{content.fields.city.label}</label>
              <input
                value={city}
            ...
            <div>
              <label className="mb-2 ml-1.5 block text-[13px] font-bold text-[#1e2364]">{content.fields.date.label}</label>
              <DatePicker value={date} onChange={setDate} placeholder={content.fields.date.placeholder} />
            </div>
            <CmsLink
              locale={locale}
