          ...
          <div className="relative h-[600px] overflow-hidden rounded-[32px] [perspective:1500px]">
            ...
            >
              <div className="device-card absolute -left-10 bottom-[-40px] right-20 top-[140px] overflow-hidden rounded-[28px] border-2 border-[#1e2364] bg-[#1e2364] p-[30px] text-white animate-[cardFloat1_5s_ease-in-out_infinite]">
                <div className="mb-2.5 text-[11px] font-bold uppercase tracking-[1.2px] text-white/85">{content.scheduleCard.label}</div>
                <div className="mb-[18px] text-[20px] font-extrabold tracking-[-0.4px]">{content.scheduleCard.detail}</div>
                <div className="flex items-center gap-2.5 rounded-[14px] border-2 border-white bg-white p-3.5">
                  ...
                  <div>
                    <strong className="block text-[14px] font-extrabold text-[#1e2364]">{content.scheduleCard.appointment.location}</strong>
                    <span className="text-[12px] text-[#6b7196]">{content.scheduleCard.appointment.detail}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="device-card absolute bottom-20 left-0 right-0 top-0 overflow-hidden rounded-[28px] border-2 border-[#1e2364] bg-[#742f88] p-6 text-white animate-[cardFloat2_7s_ease-in-out_infinite]">
                <div className="mb-2.5 text-[11px] font-bold uppercase tracking-[1.2px] text-white/85">{content.statusCard.label}</div>
                <div className="mb-[18px] text-[20px] font-extrabold tracking-[-0.4px]">{content.statusCard.status}</div>
                {content.statusCard.detail.split('·').map((statusLabel, index) => (
                  <div key={`${statusLabel}-${index}`} className="mb-2 flex items-center gap-3 rounded-[14px] border-2 border-[#1e2364] bg-white px-3.5 py-3 text-[13px] font-bold text-[#1e2364]">
                    <span
              ...
              <div className="device-card absolute bottom-5 left-[60px] right-[-40px] top-[60px] overflow-hidden rounded-[28px] border-2 border-[#1e2364] bg-white p-[26px] animate-[cardFloat3_6s_ease-in-out_infinite]">
                <div className="mb-2.5 text-[11px] font-bold uppercase tracking-[1.2px] text-[#6b7196]">{content.reportsCard.label}</div>
                <div className="mb-[18px] text-[20px] font-extrabold tracking-[-0.4px] text-[#1e2364]">{content.reportsCard.status}</div>
                {content.reportsCard.items.map((report, index) => (
                  <div key={`${report.title}-${index}`} className="mb-2 flex items-center gap-3 rounded-[14px] border-2 border-[#e5e7f0] bg-white px-3.5 py-3">
                    <div className={`flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-[10px] text-white ${index === 0 ? 'bg-[#1e2364]' : 'bg-[#742f88]'}`} aria-hidden="true">
                      {getUtilityIconByKey(report.iconKey) ?? <CheckIcon />}
                    </div>
                    <div className="min-w-0 flex-1">
                      <strong className="block text-[13px] font-extrabold text-[#1e2364]">{report.title}</strong>
                      <span className="text-[11px] text-[#6b7196]">{report.detail}</span>
                    </div>
                    <span className="rounded-full bg-[#00a8f1] px-2.5 py-1 text-[10.5px] font-extrabold text-white">{report.badge}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div>
            ...
            <h2 className="mb-7 text-[clamp(32px,4.5vw,56px)] font-extrabold leading-[1.08] tracking-[-1.6px] text-[#1e2364]">
              ...
              {content.accent ? (
                <>
                  <br />
                  <span className="font-normal italic opacity-55">{content.accent}</span>
                </>
              ) : null}
            </h2>
            <p className="text-[16px] leading-[1.65] text-[#6b7196]">{content.body}</p>
            <div className="mt-7 flex flex-col gap-3.5">
              {content.points.map(point => (
                <div key={point.title} className="flex items-center gap-3.5 rounded-[18px] border-2 border-[#e5e7f0] bg-white px-[22px] py-[18px] transition-[border-color,transform] duration-300 hover:translate-x-[3px] hover:border-[#1e2364]">
