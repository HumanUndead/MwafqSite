...
function getVisibleChildren(category: RecursiveArticleCategoryDto | null): RecursiveArticleCategoryDto[] {
  if (!category) {
    return []
  }
...
function getArticleTranslation(
  ...
  langId: number,
): Pick<ArticleTranslationDto, 'name' | 'description' | 'shortDescription' | 'extraInfo'> {
...
function mapRegisterPageContent(
  ...
  const heroArticle = getArticleByRank(rootCategory, REGISTER_PAGE_ARTICLE_RANKS.hero)
  const heroTranslation = heroArticle ? getArticleTranslation(heroArticle, langId) : null
  const stepsCategory = getChildCategoryByRank(rootCategory, REGISTER_PAGE_CHILD_CATEGORY_RANKS.steps)
  const statsCategory = getChildCategoryByRank(rootCategory, REGISTER_PAGE_CHILD_CATEGORY_RANKS.stats)
...
const fetchRegisterPageContentTree = cache(async (): Promise<RecursiveArticleCategoryDto | null> => {
  ...
  endpoint.searchParams.set('Id', String(REGISTER_PAGE_CONTENT_ROOT_CATEGORY_ID))
  ...
  try {
    ...
    console.error('[register-page-content] Failed to fetch register page content.', error)
    return null
  }
})
