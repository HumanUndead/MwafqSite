...
function emptyUserDefaults(userName: string): Omit<User, 'name' | 'username' | 'role'> {
  return {
    id: userName,
    userName,
    firstName: '',
...
export function buildUserFromToken(token: string, fallbackUserName: string): User {
  ...
  const lastName =
    readStringClaim(payload, ['family_name', 'LastName', 'lastName', 'last_name']) ?? ''
  const email =
    readStringClaim(payload, [
      'email',
      'upn',
  ...
  user.name = getUserDisplayName(user) || readStringClaim(payload, ['name', 'unique_name']) || userName
