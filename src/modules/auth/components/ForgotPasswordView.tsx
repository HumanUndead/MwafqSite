...
export function ForgotPasswordView() {
  ...
  const { step, email, loading, error, isOtpModalOpen, sendOtp, verifyOtp, resendOtp, closeOtpModal } =
    useForgotPassword()
  ...
  if (step === 'done') {
    return (
      <div className="flex flex-col items-center gap-4 text-center">
        ...
        <h2 className="text-xl font-semibold text-gray-900">{auth.forgotPassword.doneTitle}</h2>
        <p className="text-sm text-gray-600">{auth.forgotPassword.doneDescription}</p>
        <Link
