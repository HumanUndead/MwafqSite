...
export function RegisterForm() {
  ...
  const { register, loading, error, verifyOtp, closeOtpModal, resendOtp, isOtpModalOpen, verificationTarget, step } = useRegister()
  ...
  const validate = (): boolean => {
    ...
    if (form.firstName.trim().length < 2) next.firstName = auth.validation.nameMin
  ...
  const updateField = (key: keyof RegisterDto) => (e: ChangeEvent<HTMLInputElement>) => {
    ...
    return (
      <div className="flex flex-col items-center gap-4 text-center">
        <div className="flex size-16 items-center justify-center rounded-full bg-[#e8f6fd] text-[#1e2364]">
          <svg width="30" height="30" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.3" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
            <path d="M20 6 9 17l-5-5" />
          </svg>
        </div>
        <h3 className="text-2xl font-bold text-[#1e2364]">{auth.register.doneTitle}</h3>
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <div className="flex min-h-[52px] items-center justify-between gap-3 rounded-[14px] border border-[#d9ddea] bg-white px-3.5 py-2">
            <div className="min-w-0">
              <p className={cn('truncate text-sm font-medium', form.image ? 'text-[#1e2364]' : 'text-[#a3a8c4]')}>
                {selectedFileName}
              </p>
              <p className="mt-1 text-xs text-[#6b7196]">{auth.fields.profileImageHint}</p>
            </div>
            <Button
              type="button"
              variant="outline"
