  ...
  return (
    <Modal open={open} onClose={handleClose} title={auth.otp.title}>
      <div className="flex flex-col gap-5">
        ...
        <OtpInput length={otpLength} value={otp} onChange={setOtp} error={!!error} disabled={loading} />
        ...
        {onResend ? (
          <div className="text-center text-sm text-gray-500">
            {isRunning ? (
              <span>{auth.otp.resendIn.replace('{{seconds}}', formattedTime).replace('{{time}}', formattedTime)}</span>
            ) : (
              <button
                type="button"
