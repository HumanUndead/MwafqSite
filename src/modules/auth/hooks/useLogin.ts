...
export function useLogin() {
  ...
  const [otpDestinationLabel, setOtpDestinationLabel] = useState<string | null>(
    null
  );
  ...
  const verifyOtp = async (otp: string) => {
    ...
    setError(null);
    try {
      await otpApi.verifyUserNameOtp(verificationTarget, otp);
      const userResponse = await authApi.getUserByToken();
      console.log('🚀 ~ verifyOtp ~ userResponse:', userResponse);
