...
function WrenchIcon() {
  return (
    <svg className="h-16 w-16 text-[#1e2364]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
...
export default function MaintenancePage() {
  ...
  const handleRetry = async () => {
    setStatus('checking')
    try {
      const res = await fetch('/api/contact', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: '{}', signal: AbortSignal.timeout(10000) })
  ...
  return (
    <div className="flex min-h-screen flex-col items-center justify-center bg-[#f3f4f8] px-4">
      <div className="w-full max-w-md rounded-2xl bg-white p-8 text-center shadow-sm ring-1 ring-gray-100">
        ...
        <h1 className="mb-2 text-2xl font-bold text-[#1e2364]">{t.heading}</h1>
        <p className="mb-8 text-sm leading-relaxed text-gray-600">{t.description}</p>

        {status === 'down' && (
          <p className="mb-4 text-sm font-medium text-red-600">{t.stillDown}</p>
        )}
